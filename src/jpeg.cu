...
#define CUDA10U2 // our CUDA version is 12.3 so it'd definitely contain these patches
...
// -----------------------------------------------------------------------------
int decodeResizeEncodeOneImage(
    std::string sImagePath, std::string sOutputPath, double& time, int resizeWidth, int resizeHeight, int resize_quality
) {
    ...
    if (oInputStream.read(vBuffer.data(), nSize)) {
        ...
        CHECK_NVJPEG(nvjpegEncodeImage(
            nvjpeg_handle, nvjpeg_encoder_state, nvjpeg_encode_params, &imgResize, iformat, dstSize.width, dstSize.height, NULL
        ));

        // retrive the encoded bitstream for file writing
        std::vector<unsigned char> obuffer;
        size_t                     length;
        CHECK_NVJPEG(nvjpegEncodeRetrieveBitstream(nvjpeg_handle, nvjpeg_encoder_state, NULL, &length, NULL));

        obuffer.resize(length);

        CHECK_NVJPEG(nvjpegEncodeRetrieveBitstream(nvjpeg_handle, nvjpeg_encoder_state, obuffer.data(), &length, NULL));
...
int processArgs(image_resize_params_t param) {
    ...
    if (readInput(sInputPath, inputFiles)) return error_code;
...
int main(int argc, const char* argv[]) {
    ...
    params.quality = 85;
    if ((pidx = findParamIndex(argv, argc, "-q")) != -1) params.quality = std::atoi(argv[pidx + 1]);

    params.width = 0;
    if ((pidx = findParamIndex(argv, argc, "-rw")) != -1) params.width = std::atoi(argv[pidx + 1]);

    params.height = 0;
    if ((pidx = findParamIndex(argv, argc, "-rh")) != -1) params.height = std::atoi(argv[pidx + 1]);
